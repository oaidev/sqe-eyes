...
import { Users, Camera, AlertTriangle, Activity, MapPin } from 'lucide-react';
...
export default function Index() {
  ...
  const { data: zoneCount, isLoading: zl } = useQuery({
    ...
    queryFn: async () => { const { count } = await supabase.from('zones').select('*', { count: 'exact', head: true }); return count ?? 0; },
  });
  ...
  const { data: eventCount, isLoading: el } = useQuery({
    ...
    queryFn: async () => { const today = new Date().toISOString().split('T')[0]; const { count } = await supabase.from('events').select('*', { count: 'exact', head: true }).gte('detected_at', today); return count ?? 0; },
  });

  const { data: alertCount, isLoading: al } = useQuery({
    queryKey: ['stats-alerts-baru'],
    queryFn: async () => { const { count } = await supabase.from('alerts').select('*', { count: 'exact', head: true }).eq('status', 'BARU'); return count ?? 0; },
  });
    ...
    <AppLayout title="Dashboard">
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          ...
          <StatCard title="Total Zona" value={String(zoneCount ?? 0)} icon={MapPin} color="bg-green-500/10 text-green-600" loading={zl} />
          <StatCard title="Kamera Aktif" value={String(cameraCount ?? 0)} icon={Camera} color="bg-primary/10 text-primary" loading={cl} />
          <StatCard title="Event Hari Ini" value={String(eventCount ?? 0)} icon={Activity} color="bg-amber-500/10 text-amber-600" loading={el} />
